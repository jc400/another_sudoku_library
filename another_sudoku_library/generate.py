...
def _generate():
    ...
    while not done:
        ...
        for y, x in fullGen():  # loop through entire board
            n[y][x] = _picker(
                n, y, x
            )  # picker solves cell if possible, otherwise gives random
...
def _removable(board, y, x, nest=4):
    ...
    test = solve(
        test, nest=nest
    )  # test to see if board is still solveable after remove
...
def _rm(wb, forbid, ry, rx, nest=4):
    ...
    if _removable(
        wb, ry, rx, nest=nest
    ):  # if board solveable after removing this coord
...
def _carve(board, count=60):
    ...
    while removes < count and attempts < 50:
        ...
        if _rm(
            wb, forbid, ry, rx
        ):  # rm() tries remove, updates wb and forbid, returns t/f
            removes += 1
    ...
    if removes < count:
        for ry, rx in exclusiveGen():
            if _rm(
                wb, forbid, ry, rx
            ):  # nest=1):   # can use nesting to get ~5 extra carves (at time cost)
                removes += 1
                if removes == count:
                    break
